...
function loadLatestMessages(container, user_id)
{
    ...
    chat_area.html("");
    console.log('fetch for latest msg', container, user_id)
    ...
    $.ajax({
        ...
        success: function (response) {
            console.log('resposne state', response.messages)
            if(response.state == 1) {
...
{
    console.log(message);
    var image =  base_url  +  '/images/user-avatar.png';
    if (message.from_user.profile_image){
        image = message.from_user.profile_image
    }
    ...
    return `
        ...
        <div class="col-md-2 col-xs-2 col-2 avatar px-0">
            <img src="${image}" style="width: 40px; height: 40px; border-radius: 50%;">
        </div>
    </div>
    `;
}
...
{
    var image =  base_url  +  '/images/user-avatar.png';
    if (message.to_user.profile_image){
        image = message.to_user.profile_image
    }
    
    return `
           <div class="row msg_container base_receive" data-message-id="${message.id}">
           <div class="col-md-2 col-xs-2 col-2 avatar px-0">
             <img src="${image}" width="40" height="40" class="img-responsive">
           </div>
        <div class="col-md-10 col-xs-10 col-10">
